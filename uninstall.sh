...
if [ -x "$SCRIPT_DIR/touchslide-config" ]; then
    "$SCRIPT_DIR/touchslide-config" restore-all >/dev/null 2>&1 || true
    sleep 1
elif command -v touchslide-config >/dev/null 2>&1; then
    touchslide-config restore-all >/dev/null 2>&1 || true
    sleep 1
fi
...
if [ -f "$HOME/.config/kglobalshortcutsrc" ]; then
    cp "$HOME/.config/kglobalshortcutsrc" "$HOME/.config/kglobalshortcutsrc.bak-touchslide-uninstall"
    perl -0pi -e 's/^Touch Slide Window(?: V[0-9]+)?:[^\n]*\n//mg' "$HOME/.config/kglobalshortcutsrc"
fi

if [ -f "$HOME/.config/kwinrc" ]; then
    cp "$HOME/.config/kwinrc" "$HOME/.config/kwinrc.bak-touchslide-uninstall"
